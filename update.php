...
$scheduleArray = Array();
$db            = mysql_connect( /* your database address */, /* your database username */, /* your database password */ );
mysql_select_db( /* your database table name */, $db );
	...
	if ( strpos( $item->title, "Public Schedule" ) != FALSE ) {
		...
		if ( $results == FALSE ) {
			foreach ( $export[ "entries" ] as $entry ) {
				$query = mysql_query( "INSERT INTO pedutoSchedule (title, start, end, location, date) values ('" . mysql_real_escape_string( $entry[ "title" ] ) . "', '" . $entry[ "start" ] . "', '" . $entry[ "end" ] . "', '" . mysql_real_escape_string( $entry[ "location" ] ) . "', '